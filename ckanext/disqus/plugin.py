...
class Disqus(p.SingletonPlugin):
    ...
    def configure(self, config):
        ...
        if disqus_name is None:
            log.warn(
                "No disqus forum name is set. Please set \
                'disqus.name' in your .ini!"
            )

        disqus_developer = p.toolkit.asbool(
            config.get("disqus.developer", "false")
        )
    ...
    def disqus_comments(cls):
        ...
        try:
            identifier = tk.c.controller
            if tk.c.current_package_id:
                identifier += "::" + tk.c.current_package_id
            elif tk.c.id:
                identifier += "::" + tk.c.id
            else:
                ...
                identifier = ""
            # special case
            if tk.c.action == "resource_read":
                identifier = "dataset-resource::" + tk.c.resource_id
        except:
            identifier = ""
        data = {
        ...
        return tk.render_snippet("disqus_comments.html", data)
    ...
    def disqus_recent(cls, num_comments=5):
        ...
        return tk.render_snippet("disqus_recent.html", data)
    ...
    def current_disqus_url(cls,):
        ...
        return tk.h.url_for_static_or_external(
            tk.request.environ["CKAN_CURRENT_URL"],
            qualified=True,
            host=cls.disqus_url,
        )
